...
barplot.mrocalc <- function(obj) {
  ...
  tops <- apply(matrix(rep(1,nlevels(s1$var)),nrow=1), 2, function(x) rbind(0, x, x, 0))
  ymax <- c(tops)
  ...
  grid.polygon(unit(xx, "native"), unit(ymax, "native"), id = id,
               gp =
                 gpar(fill = "grey", col = "#000000",  # fill = grey first time, red or whatever the second ...
                      lwd = opts$bar.lwd))
  
  grid.polygon(unit(xx, "native"), unit(yy, "native"), id = id,
               gp =
                 gpar(fill = "red", col = "#000000",  # fill = grey first time, red or whatever the second ...
                      lwd = opts$bar.lwd))
  ...
  lapply(1:length(s1$compL),function(i){
    grid.abline(intercept=unit(s1$compL[i], "native"), slope= unit(0,"native"), 
                gp=gpar(col = "gold", lty = 2))
    grid.abline(intercept=unit(s1$compU[i], "native"), slope= unit(0,"native"), 
                gp=gpar(col = "gold", lty = 2))
  }
  )
  
  grid.segments(x0 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y0 = unit(s1$compL, "native"),
                x1 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y1 = unit(s1$compU, "native"),
                gp =
                  gpar(col = "green",  
                       lwd = opts$inf.lwd.comp))
  
  grid.segments(x0 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y0 = unit(s1$confL, "native"),
                x1 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y1 = unit(s1$confU, "native"),
                gp =
                  gpar(col = "black",  
...
barplot.bymrocalc <- function(obj,g1.level = NULL, g2.level ="_MULTI",...){
    ...
    if (which((names(s1) == "var"))<3){
      ...
      for (i in 1:nrow(praw)) {
        ...
        lapply(1:length(compL),function(i){
          grid.abline(intercept=unit(compL[i], "native"), slope= unit(0,"native"), 
                      gp=gpar(col = "gold", lty = 2))
          grid.abline(intercept=unit(compU[i], "native"), slope= unit(0,"native"), 
                      gp=gpar(col = "gold", lty = 2))
        }
        )
        
        
        grid.segments(x0 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y0 = unit(compL, "native"),
                      x1 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y1 = unit(compU, "native"),
                      gp =
                        gpar(col = "green",  # fill = grey first time, red or whatever the second ...
                             lwd = opts$inf.lwd.comp))
        
        grid.segments(x0 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y0 = unit(confL, "native"),
                      x1 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y1 = unit(confU, "native"),
                      gp =
                        gpar(col = "black",  # fill = grey first time, red or whatever the second ...
                             lwd = opts$inf.lwd.conf))
      ...
      for (j in 1:nlevels(s1$type2)) {
        for (i in 1:nlevels(s1$type1)) {
          ...
          compL <- matrix(s1$compL, nrow=nlevels(s1$type1)*nlevels(s1$type2), byrow=TRUE)[k,]
          compU <- matrix(s1$compU, nrow=nlevels(s1$type1)*nlevels(s1$type2), byrow=TRUE)[k,]
          confL <- matrix(s1$confL, nrow=nlevels(s1$type1)*nlevels(s1$type2), byrow=TRUE)[k,]
          confU <- matrix(s1$confU, nrow=nlevels(s1$type1)*nlevels(s1$type2), byrow=TRUE)[k,]
...
barplot.between <- function(obj) {
  ...
  grid.polygon(unit(xx, "native"), unit(yy, "native"), 
               id = id,
               gp =
                 gpar(fill = colz, col = "#000000",  # fill = grey first time, red or whatever the second ...
                      lwd = opts$bar.lwd))
  ...
  lapply(1:ncol(group.range), function(j) {
    
    left = group.range[1,j]
    right = group.range[2,j]
    grid.segments(x0 = unit(left, "native"), y0 = unit(compL[j,], "native"),
                  x1 = unit(right, "native"), y1 = unit(compL[j,], "native"),
                  gp =
                    gpar(col = "gold",  # fill = grey first time, red or whatever the second ...
                         lty = 2))
    grid.segments(x0 = unit(left, "native"), y0 = unit(compU[j,], "native"),
                  x1 = unit(right, "native"), y1 = unit(compU[j,], "native"),
                  gp =
                    gpar(col = "gold",  # fill = grey first time, red or whatever the second ...
                         lty = 2))
  })
  ...
  grid.segments(x0 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y0 = unit(s2$confL, "native"),
                x1 = unit(rowMeans(medpoint.mat[,2:3]), "native"), y1 = unit(s2$confU, "native"),
                gp =
                  gpar(col = "black",  # fill = grey first time, red or whatever the second ...
                       lwd = opts$inf.lwd.conf))
...
barplot.b2 <- function(obj, g1.level=NULL) {
  ...
  x <- factor(rep(levels(s2$var)[1], nlevels(s2$type1)*nlevels(s2$type2)), 
              levels = levels(s2$var))
  y <- factor(rep(unique(as.character(s2$type1)),each=nlevels(s2$type2)),
...
#     partial.key <- which(g1 %in% g1.level)
  ...
  for (i in 1:nrow(praw)){
    ...
    compL <- matrix(matrix(s2$compL, nrow=nlevels(s2$type2),byrow=TRUE)[i,],nrow=1)
    compU <- matrix(matrix(s2$compU, nrow=nlevels(s2$type2),byrow=TRUE)[i,],nrow=1)
    confL <- matrix(matrix(s2$confL, nrow=nlevels(s2$type2),byrow=TRUE)[i,],nrow=1)
    confU <- matrix(matrix(s2$confU, nrow=nlevels(s2$type2),byrow=TRUE)[i,],nrow=1)
    ...
    lapply(1:ncol(group.range), function(k) {
      left = group.range[1,k]
      right = group.range[2,k]
      grid.segments(x0 = unit(left, "native"), y0 = unit(lower.range[k,], "native"),
                    x1 = unit(right, "native"), y1 = unit(lower.range[k,], "native"),
                    gp =
                      gpar(col = "gold", 
                           lty = 2))
      grid.segments(x0 = unit(left, "native"), y0 = unit(upper.range[k,], "native"),
                    x1 = unit(right, "native"), y1 = unit(upper.range[k,], "native"),
                    gp =
                      gpar(col = "gold", 
                           lty = 2))
