...
plot.bymrocalc <- function(x, show = NULL, ...) {
    ...
    if (is.null(show)) {
        ...
        opar <- par(mfrow = c(1, length(show)))
        dev.hold()
        for (j in show) {
            barplot(x[[j]], ...)
            mtext(names(x)[j])
...
plot.b2 <- function(x, LEG, which = NULL, ...) {
    obj <- x
    ...
    for(i in 1:Lead) {
      barplot.between(obj[[i]], main = Name[i], LEG)  
    }
    plot.new()
...
summary.within <- function(object, ...) {
    Title <-
        if (length(dim(object)) < 2) {
            paste0(names(dimnames(object)), ":", dimnames(object))
        } else {
            commonTitle <- 
                paste0(names(dimnames(object)[1]), ":", dimnames(object)[2])
...
mranalysis <- function(expr, base, ...) {
    ...
    if (con[1] == "|") {
        ...
        out2$within <- by(returnMro(out$lhs, base), out$condition,
                          mroPara, nonparallel = deparse(substitute(expr)))
        out2$between <- between(out2$within)
        summary(out2$between, out2$within)
        dev.new()
        barplot(out2$between, ...)
    }
}
...
returnMro <- function(callname, listmro) {
  ...
  if (sum(index) != 1) 
    stop(paste(as.character(callname), "does not exist in ",
         substitute(listmro)))
